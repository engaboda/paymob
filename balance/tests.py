...
class TestPromoCode(APITestCase):
    ...
    def test_valid_code(self):
        ...
        code = mommy.make(PromoCode, frequency_of_use=5, benefit=5, quantity=5, code='1234abc', category='A', start_date=timezone.now().date(), end_date=timezone.now().date() + monthdelta(1))
        products = mommy.make(Product, _quantity=5, category='A')
        data = {
            'code': code.code,
            'product': [product.id for product in products]
        }
        url = '/en/paymob_game/promo_code/'
    ...
    def test_invalid_code(self):
        user = mommy.make(User)
        mommy.make(PromoCode, frequency_of_use=5, benefit=5, quantity=5, code='1234abc', category='A', start_date=timezone.now().date(), end_date=timezone.now().date() + monthdelta(1))
        products = mommy.make(Product, _quantity=5, category='A')
        data = {
            'code': 'BAD',
            'product': [product.id for product in products]
    ...
    def test_valid_code_ar(self):
        user = mommy.make(User)
        code = mommy.make(PromoCode, frequency_of_use=5, benefit=5, quantity=5, code='ahmed', category='A', start_date=timezone.now().date(), end_date=timezone.now().date() + monthdelta(1))
        products = mommy.make(Product, _quantity=5, category='A')
        data = {
            'code': code.code_ar,
            'product': [product.id for product in products]
